 ...
 * sharing the source's blocks: otherwise holes in the source leave the
 * destination's pre-clone data in place (the dest must mirror the source
 * exactly, including its holes — caught by fsx).
 *
 * Block-aligned range only (reflink shares whole blocks).  Caller holds
...
int ocsfs_clear_block_range(struct inode *inode, u64 start_block, u64 end_block)
{
	...
	/* Walk backwards (safe for removal/shrink); same four overlap cases as
	 * ocsfs_punch_hole, but free refcount-aware. */
	for (i = oi->i_extent_count - 1; i >= 0; i--) {
		struct ocsfs_extent *e = &oi->i_extents[i];
		u64 ext_start = e->logical_block;
		u64 ext_end = e->logical_block + e->length;
		...
		if (ext_start >= start_block && ext_end <= end_block) {
			...
			/* Head overlap: range covers the front; shrink from front. */
			u32 removed = (u32)(end_block - ext_start);
			...
			/* Tail overlap: range covers the back; shrink from back. */
			u32 removed = (u32)(ext_end - start_block);
	...
	/* Journal the inode with the updated extent map BEFORE freeing blocks
	 * (journal-before-free: no cross-link on crash). */
	if (nfrees > 0) {
		int fr = ocsfs_flush_inode_locked(inode, false);
