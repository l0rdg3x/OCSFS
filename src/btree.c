...
/*
 * Split a leaf node. The right half goes to a new node.
 * Returns the new node's block number and the split key (first key of new node).
 */
static int split_leaf(struct ocsfs_btree *bt, void *buf,
                       uint64_t *out_new_block, uint64_t *out_split_key)
{
    struct ocsfs_btree_node_hdr *hdr = node_hdr(buf);
    struct ocsfs_btree_entry *entries = leaf_entries(buf);
    int count = hdr->bn_count;
    int split = count / 2;

    /* Allocate new leaf */
    uint64_t new_block;
    int ret = alloc_node(bt, &new_block);
    if (ret < 0) return ret;
    ...
    int new_count = count - split;
    memcpy(new_entries, &entries[split], new_count * sizeof(struct ocsfs_btree_entry));
    new_hdr->bn_count = new_count;
    ...
    /* If there was a right sibling, update its left link */
    ...
    *out_split_key = new_entries[0].key;
 ...
 */
static int split_internal(struct ocsfs_btree *bt, void *buf,
                           uint64_t *out_new_block, uint64_t *out_promote_key)
{
    struct ocsfs_btree_node_hdr *hdr = node_hdr(buf);
    uint64_t *first_child = internal_first_child(buf);
    struct ocsfs_btree_ptr *ptrs = internal_ptrs(buf);
    ...
    *out_promote_key = ptrs[split].key;
    ...
    init_internal_node(new_buf, bt->block_size, new_block, hdr->bn_level);

    struct ocsfs_btree_node_hdr *new_hdr = node_hdr(new_buf);
    uint64_t *new_first_child = internal_first_child(new_buf);
    struct ocsfs_btree_ptr *new_ptrs = internal_ptrs(new_buf);

    /* New node gets: first_child = ptrs[split].child, then ptrs[split+1..count-1] */
    *new_first_child = ptrs[split].child;
    int new_count = count - split - 1;
    if (new_count > 0) {
        memcpy(new_ptrs, &ptrs[split + 1], new_count * sizeof(struct ocsfs_btree_ptr));
    }
    new_hdr->bn_count = new_count;
    new_hdr->bn_parent = hdr->bn_parent;

    /* Original node keeps first_child and ptrs[0..split-1] */
    hdr->bn_count = split;
    (void)first_child; /* first_child stays the same */
