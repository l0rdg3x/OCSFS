 ...
 *
 * ocsfs2_bsg_execute_cdb() is the single entry point for both paths.
 * On non-SCSI devices (virtio-blk, loop, NVMe) both probes fail gracefully
 * and ocsfs2_atomic_cas() falls back to the PR-lease software path.
 */
...
#define OCSFS2_CAW_BUF_SIZE  8192U   /* 2 × 4096-byte max sector */
#define OCSFS2_CAW_POOL_MIN  4       /* guaranteed elements under memory pressure */

static struct kmem_cache *ocsfs2_caw_cache;
static mempool_t         *ocsfs2_caw_pool;

int ocsfs2_scsi_pool_init(void)
{
	ocsfs2_caw_cache = kmem_cache_create("ocsfs2_caw_buf", OCSFS2_CAW_BUF_SIZE,
					    0, SLAB_HWCACHE_ALIGN, NULL);
	if (!ocsfs2_caw_cache)
		return -ENOMEM;

	ocsfs2_caw_pool = mempool_create_slab_pool(OCSFS2_CAW_POOL_MIN,
						   ocsfs2_caw_cache);
	if (!ocsfs2_caw_pool) {
		kmem_cache_destroy(ocsfs2_caw_cache);
		ocsfs2_caw_cache = NULL;
		return -ENOMEM;
	}
	return 0;
}
...
{
	mempool_destroy(ocsfs2_caw_pool);
	ocsfs2_caw_pool = NULL;
	kmem_cache_destroy(ocsfs2_caw_cache);
	ocsfs2_caw_cache = NULL;
}
...
static enum pr_type ocsfs2_to_pr_type(u8 t)
{
	switch (t) {
	case OCSFS2_PR_TYPE_WRITE_EXCL:      return PR_WRITE_EXCLUSIVE;
	case OCSFS2_PR_TYPE_EXCL_ACCESS:     return PR_EXCLUSIVE_ACCESS;
	case OCSFS2_PR_TYPE_WRITE_EXCL_REG:  return PR_WRITE_EXCLUSIVE_REG_ONLY;
	case OCSFS2_PR_TYPE_EXCL_ACCESS_REG: return PR_EXCLUSIVE_ACCESS_REG_ONLY;
	default:
		WARN_ON(1);
...
static const struct pr_ops *ocsfs2_pr_ops(struct super_block *sb)
{
 ...
 */
static inline int ocsfs2_pr_norm(int ret)
{
...
int ocsfs2_pr_register(struct super_block *sb, u64 key)
{
	...
	const struct pr_ops *ops = ocsfs2_pr_ops(sb);
	int ret = 0;
...
int ocsfs2_pr_unregister(struct super_block *sb)
{
	...
	if (ops && ops->pr_register)
		ret = ocsfs2_pr_norm(ops->pr_register(sb->s_bdev,
						     sbi->s_pr.pr_key, 0, 0));
...
int ocsfs2_pr_reserve(struct super_block *sb, u8 type)
{
	...
	if (!ops || !ops->pr_reserve)
		return 0;
	return ocsfs2_pr_norm(ops->pr_reserve(sb->s_bdev, sbi->s_pr.pr_key,
					     ocsfs2_to_pr_type(type), 0));
}
...
int ocsfs2_pr_release(struct super_block *sb, u8 type)
{
	...
	if (!ops || !ops->pr_release)
		return 0;
	return ocsfs2_pr_norm(ops->pr_release(sb->s_bdev, sbi->s_pr.pr_key,
					     ocsfs2_to_pr_type(type)));
}
...
int ocsfs2_pr_preempt(struct super_block *sb, u64 victim_key, u8 type)
{
	...
	if (!ops || !ops->pr_preempt)
		return 0;
	return ocsfs2_pr_norm(ops->pr_preempt(sb->s_bdev, sbi->s_pr.pr_key,
					     victim_key,
					     ocsfs2_to_pr_type(type), false));
}
...
/*
 * ocsfs2_bsg_get_sdev — obtain scsi_device from block device queue.
 *
 ...
 */
static struct scsi_device *ocsfs2_bsg_get_sdev(struct request_queue *q)
{
...
typedef struct scsi_device *(*ocsfs2_sdfq_fn_t)(struct request_queue *q);
static ocsfs2_sdfq_fn_t ocsfs2_sdfq;
 ...
 */
static bool ocsfs2_caw_resolve_sdfq(void)
{
	...
	ocsfs2_sdfq = (ocsfs2_sdfq_fn_t)kp.addr;
	unregister_kprobe(&kp);
	return ocsfs2_sdfq != NULL;
}

/* ── Unified probe ── */

/*
 * ocsfs2_scsi_caw_probe — check whether CAW is available on this device.
 *
...
/* ── ocsfs2_bsg_execute_cdb — unified SCSI CDB execution ── */

/*
 * ocsfs2_bsg_execute_cdb — execute an arbitrary SCSI CDB via scsi_execute_cmd.
 *
 ...
 */
int ocsfs2_bsg_execute_cdb(struct super_block *sb,
...
/* ── ocsfs2_scsi_caw — public CAW entry point ── */
 ...
 */
int ocsfs2_scsi_caw(struct super_block *sb, u64 lba,
		   ...
		   unsigned int lbs)
{
	...
	if (2 * lbs <= OCSFS2_CAW_BUF_SIZE && ocsfs2_caw_pool) {
		buf = mempool_alloc(ocsfs2_caw_pool, GFP_NOIO);
	} else {
	...
	ocsfs2_build_caw_cdb(cdb, lba * num_blocks, num_blocks);

	ret = ocsfs2_bsg_execute_cdb(sb, cdb, buf, 2 * lbs, DMA_TO_DEVICE);

	if (2 * lbs <= OCSFS2_CAW_BUF_SIZE && ocsfs2_caw_pool)
		mempool_free(buf, ocsfs2_caw_pool);
	else
		kfree(buf);
